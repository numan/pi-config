...
| Live trace, LCP attribution, INP attribution, layout shift attribution | Chrome DevTools MCP server (`performance_*`, `lighthouse_audit`) | `chrome-devtools` MCP server configured in the harness; for Pi browser automation workflows, see `agent-browser` |
...
8. Use `docs/references/performance-checklist.md` as the minimum baseline for each area.
...
- **Do not invoke from another persona.** If `reviewer` flags a performance concern that warrants a deeper pass, surface that recommendation in the report; the user or a slash command initiates the deeper pass. See upstream agent composition notes at `docs/agents.md`.
---

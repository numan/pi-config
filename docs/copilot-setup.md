...
cat /path/to/agent-skills/skills/code-reviewer/SKILL.md > .github/skills/code-reviewer/SKILL.md
```
...
cp /path/to/agent-skills/agents/reviewer.md .github/agents/reviewer.agent.md
...
2. **Use agents for review** — The reviewer, test-engineer, and security-auditor agents are designed for Copilot's agent model.
3. **Reference in chat** — When working on a specific phase, paste the relevant skill content into Copilot Chat for context.
4. **Combine with PR reviews** — Set up Copilot to review PRs using the reviewer agent persona.

...
echo -e "\n---\n" >> GEMINI.md
cat /path/to/agent-skills/skills/code-reviewer/SKILL.md >> GEMINI.md
```
...
- `agent-browser` uses the `chrome-devtools` MCP extension.
...
3. **Use agents for review** — Copy `agents/reviewer.md` content when requesting structured code reviews.

...
3. **code-reviewer** — For verifying quality before merge
...
```
Starting a project:  spec-driven-development → write-todos
During development:  incremental-implementation + test-driven-development
Before merge:        code-reviewer + security-and-hardening
...
| Agent | Purpose |
|-------|---------|
| `reviewer.md` | Five-axis code review |
...
Load an agent definition when you need specialized review. For example, ask your coding agent to "review this change using the reviewer agent persona" and provide the agent definition.
...
| `/build auto` | write-todos → incremental-implementation + test-driven-development (whole plan, one approval) |

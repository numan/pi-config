...
```
/review → reviewer (with code-reviewer skill) → report
```
...
Plugin subagents go in `agents/` at the plugin root. This repo is a plugin (`.claude-plugin/plugin.json`), so `agents/reviewer.md`, `agents/security-auditor.md`, and `agents/test-engineer.md` are auto-discovered when the plugin is enabled. No path configuration needed.
...
Don't redefine these. Layer your specialist personas (reviewer, security-auditor, test-engineer) on top of them.
...
The fields that DO work in plugin agents are: `name`, `description`, `tools`, `disallowedTools`, `model`, `maxTurns`, `skills`, `memory`, `background`, `effort`, `isolation`, `color`, `initialPrompt`. Use `model` per-persona if you want to optimize cost (e.g. Haiku for `test-engineer` coverage scans, Sonnet for `reviewer`, Opus for `security-auditor`).
...
three teammates using the existing agent types:
  ...
  - reviewer       — investigate race conditions and blocking calls
...
4. When `reviewer` finds a `Promise.all` that should be sequential, it messages `security-auditor` to confirm the auth call isn't part of the race. `security-auditor` checks and replies — either confirming the race is the real issue or producing counter-evidence.
...
```
/work → router-persona → "this needs a review" → reviewer → router (paraphrases) → user
```
...
A `reviewer` that internally invokes `security-auditor` when it sees auth code.
...
`/ship` calls a `pre-ship-coordinator` that calls a `quality-coordinator` that calls `reviewer`.

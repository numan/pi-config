...
4.  **Audit Each Document:**
    ...
    - **Verify Code Accuracy (if applicable):**
      - If the document contains code snippets (e.g., shell commands, API calls,
        file paths, Docker image versions), use `rg` through `bash` and the
        `read` tool within the `packages/` directory (or other relevant parts
        of the codebase) to ensure the code is still accurate and up-to-date. Pay close
...
3.  **Implement Changes:**
    - For each approved recommendation:
      ...
      - Apply the recommended change using the `edit` tool. Pay close
        attention to `oldText` for exact matches, including whitespace and
        newlines. For multiple occurrences of the same simple string, use one
        edit entry per verified occurrence or a carefully scoped replacement.
      ...
      - For structural changes, such as adding an overview paragraph, use
        `edit` or `write` as appropriate.
      - For broken links, determine the correct new path or update the link
        text.
      - For creating new files, such as `docs/get-started.md` to fix a broken
        link, or a new feature article, use `write`.
